...
- WebSocket receive pass: Mattermost docs describe `/api/v4/websocket`, but hosted Infomaniak kChat exposes an Echo/Pusher socket at `websocket.kchat.infomaniak.com` with `/broadcasting/auth`. Added `receiveMode: "websocket"` support through OpenClaw `gateway.startAccount`, automatic team/user ID discovery, private-team and presence-teamUser subscription auth, and `posted` event dispatch through the same inbound runtime path as webhooks. Live smoke subscribed to the test workspace, posted one message, received it over WebSocket, and dispatched inbound id `019ec227-1886-71da-941b-46b10c91875e`.
